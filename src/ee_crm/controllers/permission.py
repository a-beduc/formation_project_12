...
# RBAC args decorator permission, ABAC using current predicate system after
# if Role.DEACTIVATED ; ["ALL"]
# if Role.SALES ; ["ALL", "COLLABORATOR", "SALES"]
# ...
# Flow idea :
# get token role, use Role(token_role).name >>> "SALES"
# "SALES" >>> perms in ["ALL", "COLLABORATOR", "SALES"]
# in @permission, check if *args perms in list (RBAC)
# then check if resources satisfy requirements predicate (old system) (ABAC)
# proceed
perms = {
    "ALL": {
    ...
    "MANAGEMENT": {
        ...
        "contract:create",
        "contract:update_unassigned",
        "contract:delete_unassigned",
        "event:modify_support",
        "event:delete_unassigned"
    },
    "SALES": {
        "client:create",
        "client:update_own",
